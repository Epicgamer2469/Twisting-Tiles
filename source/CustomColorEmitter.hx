...
typedef CustomColorEmitter = FlxTypedEmitter<FlxParticle>;
...
#end
{
	 ...
	 * @return  This `FlxEmitter` instance (nice for chaining stuff together).
	 */
	public function makeParticles(Width:Int = 2, Height:Int = 2, Quantity:Int = 50):FlxTypedEmitter<T>
	{
		var colorArray:Array<String> = ['RED', 'ORANGE', 'YELLOW', 'GREEN', 'BLUE', 'PURPLE'];
		maxSize = Quantity;

		for (i in 0...Quantity)
		{
			var particle:T = Type.createInstance(particleClass, []);
			particle.makeGraphic(Width, Height, FlxColor.fromString(colorArray[FlxG.random.int(0, 5)]));
			add(particle);
		}
